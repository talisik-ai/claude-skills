# prd-assessment v2 Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Rewrite `prd-assessment` SKILL.md from a PRD writer into a 3-mode skill: CREATE (interview → new PRD), ASSESS (existing PRD → impact assessment), and PIVOT (direction change → impact assessment with invalidation sections).

**Architecture:** Single SKILL.md file, fully self-contained. Mode is auto-detected from conversation context — no flags or commands needed. The registry.json entry is updated to reflect the new version and description.

**Tech Stack:** Markdown, YAML frontmatter. No code changes — this is a skill content rewrite.

**Design doc:** `docs/plans/2026-03-07-prd-assessment-rewrite-design.md`
...
**Files:**
- Modify: `packages/skills/prd-assessment/SKILL.md` (full rewrite)

**Step 1: Read the current file**

Run: `cat packages/skills/prd-assessment/SKILL.md`
Confirm you understand what the current skill does (interview-driven PRD writer).

**Step 2: Replace with the new content**

Write the following complete content to `packages/skills/prd-assessment/SKILL.md`:
...
1. **Read the PRD file.** If it doesn't exist, stop and ask for the correct path.
...
Check the file starts with the correct frontmatter and contains all three mode sections:
...
git commit -m "feat: rewrite prd-assessment as 3-mode skill (create/assess/pivot)
...
Confirm the current `prd-assessment` entry before editing.

**Step 2: Update the entry**

Find the `prd-assessment` object and update these fields:

```json
{
  "name": "prd-assessment",
  "displayName": "PRD Assessment",
  "version": "2.0.0",
  "description": "Three-mode PRD skill: CREATE (interview → new PRD for greenfield projects), ASSESS (existing PRD + architecture → IMPACT-ASSESSMENT.md), PIVOT (direction change → impact assessment with skills to disable, invalidated context MDs, and affected in-flight cards).",
  "author": "talisik-ai",
  "category": "product",
  "tags": ["prd", "product", "impact-assessment", "pivot", "planning", "requirements"],
  "path": "packages/skills/prd-assessment",
  "installSize": "14kb",
  "requiresTools": [],
  "readme": "https://github.com/talisik-ai/claude-skills/tree/main/packages/skills/prd-assessment"
}
```
...
git commit -m "chore: bump prd-assessment to v2.0.0 in registry
